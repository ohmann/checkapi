...
static int test_tempdir(const char *temp_dir, apr_pool_t *p)
{
    ...
    if (apr_file_mktemp_log(&dummy_file, path, 0, p, NESTED) == APR_SUCCESS) {
        if (apr_file_putc_log('!', dummy_file, NESTED) == APR_SUCCESS) {
            if (apr_file_close_log(dummy_file, NESTED) == APR_SUCCESS) {
                return 1;
            }
        }
    }
