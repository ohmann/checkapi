...
# EG:
dy_import_module_symbols("lind_fs_constants.py")
dy_import_module_symbols("fs_net_handler.repy")
dy_import_module_symbols("serialize.repy")
dy_import_module_symbols("parser_intermediate_representation.py")
...
def persist_metadata(metadatafilename):
  # JR: Ignoring persisting metadata to disk.
  return
  ...
  try:
    ...
    # if the file did exist, were we told to create with exclusion?
      ...
      if 'O_TRUNC' in flags:
        ...
        # remove the file...
        model_removefile('MainThread', FILEDATAPREFIX+str(inode))

        # always open the file.
        fileobjecttable[inode] = model_openfile('MainThread', FILEDATAPREFIX+str(inode),True)
    ...
    if IS_REG(filesystemmetadata['inodetable'][inode]['mode']):
      ...
      if inode not in fileobjecttable:
        thisfo = model_openfile('MainThread', FILEDATAPREFIX+str(inode),False)
...
def write_syscall(fd, data, count):
  ...
  try:
    ...
    if blankbytecount > 0:
      ...
      model_file_writeat('MainThread', fileobjecttable[inode], '\0'*blankbytecount, filesize)
      

    # writeat never writes less than desired in Repy V2.
    #fileobjecttable[inode].writeat(data,position)
    model_file_writeat('MainThread', fileobjecttable[inode], data, position)
...
def _close_helper(fd):
  # JR: Added support for closing net fds.
  # Are we dealing with a file-system fd?
  if 'inode' in filedescriptortable[fd]:
    # get the inode for the filedescriptor
    inode = filedescriptortable[fd]['inode']

  # No go on file-system fd, must be network!
  else:
  ...
  #fileobjecttable[inode].close()
  model_file_close('MainThread', fileobjecttable[inode])
